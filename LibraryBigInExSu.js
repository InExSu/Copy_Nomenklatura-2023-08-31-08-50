...
function duplicatesColumnEmail(sheetName, rowNumber, headerTitle, regexPattern, email1, email2) {
  ...
  if (subject !== '') {
    subject += ' ' + SpreadsheetApp.getActiveSpreadsheet().getName() + ' | ' + sheet.getName();
    MailApp.sendEmail(email, subject, body);
  }
}

/** 
 * Столбец в массив - формулы и значения
 */
function formulasAndValuesToArrays(sheet, column) {
  var lastRow = sheet.getLastRow();

  // Получаем формулы и значения из столбца
  var formulas = sheet.getRange(1, column, lastRow).getFormulasR1C1();
  var values = sheet.getRange(1, column, lastRow).getValues();

  // Создаем массив для хранения формул и значений
  var formulasAndValues = [];
  var formula = '';
  var value = '';

  // Объединяем формулы и значения в массив
  for (var i = 0; i < formulas.length; i++) {

    formula = formulas[i][0];
    value = values[i][0];

    // Если есть формула, то добавляем ее, иначе добавляем значение
    if (formula) {
      formulasAndValues.push([formula]);
    } else {
      formulasAndValues.push([value]);
    }
  }

  return formulasAndValues;
}

function SheetCellFormatDecor() {

  let msg = "Обновляю форматирование листов ...";
  SpreadsheetApp.getActiveSpreadsheet().toast(msg);

  const spreadSourceID = "19SvdLfCv-OM9yBSG5ojbDc3UdZA-VqGH1ULtqZgB1AY"; // Номенклатура

  SheetCellFormatCopy(spreadSourceID, 'Прайс партнеры без НДС', 'Прайс партнеры без НДС');
  SheetCellFormatCopy(spreadSourceID, 'Прайс партнеры c НДС', 'Прайс партнеры c НДС');

  msg = "Обновление форматирования листов завершено";
  SpreadsheetApp.getActiveSpreadsheet().toast(msg);

}

/** 
 * Копировать формат листа из одной таблицы на лист другой таблицы
 */
function SheetCellFormatCopy(spreadSourceID, sheetSourceName, sheetDestName) {

  var spreadDest = SpreadsheetApp.getActive();
  var sheetDest = spreadDest.getSheetByName(sheetDestName);
  var rangeDest = sheetDest.getDataRange();

  var spreadSource = SpreadsheetApp.openById(spreadSourceID);
  var sheetSource = spreadSource.getSheetByName(sheetSourceName);

  // Скопировать лист-источник во временный лист в книге-приёмнике
  // copyFormatToRange требует, чтобы листы были в одной книге 
  var sheetTemp = sheetSource.copyTo(spreadDest);

  // Копируем формат из временного листа в лист Dest
  var tempRange = sheetTemp.getRange("A:R");
  // copyFormatToRange(sheet, column, columnEnd, row, rowEnd)
  tempRange.copyFormatToRange(sheetDest, 1, rangeDest.getLastColumn(), 1, rangeDest.getLastRow());

  // Удаляем временный лист
  spreadDest.deleteSheet(sheetTemp);
}
