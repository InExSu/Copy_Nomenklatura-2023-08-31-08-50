function onOpen() {
  ...
  ui.createMenu('Прайсы')
    ...
    .addItem('Ссылки сделать активными на листе сводная таблица', 'pivot_Links_Activate')
...
function onEdit(event) {
  ...
  if (sheetName == 'Прайс без НДС') {
    // в модуле PricePaint_Chain
...
function sKUs_FormatCopy() {

  var spread = SpreadsheetApp.getActive();

  const msg = 'Обновляю формат артикулов с листа \n' +
    'Прайс без НДС на лист \n' +
    'Прайс партнеры без НДС';
  spread.toast(msg);

  spread.getRange('\'Прайс без НДС\'!L:R').copyTo(spread.getRange('\'Прайс партнеры без НДС\'!L:R'), SpreadsheetApp.CopyPasteType.PASTE_FORMAT, false);
}
